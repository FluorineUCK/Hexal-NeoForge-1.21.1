...
/** Arithmetic exposed by gate iotas. */
object GateArithmetic : Arithmetic {
    private val OPS = listOf(ABS)
    ...
    private fun make1Double(op: Function<GateIota, Double>): OperatorUnary =
        OperatorUnary(all(ofType(GATE))) { i: Iota ->
            DoubleIota(op.apply(Operator.downcast(i, GATE)))
        }
}
