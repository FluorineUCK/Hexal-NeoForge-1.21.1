...
object OpFreeze : SpellAction {
	...
	override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
		...
		return SpellAction.Result(
			Spell(toFreeze),
			Hexal.toMediaFromDust(HexalConfig.Server.FREEZE_COST.get()),
	...
	private data class Spell(val vec: Vec3) : RenderedSpell {
		override fun cast(env: CastingEnvironment) {
			...
			val recipe = recipes.find { it.value.matches(blockState) }

			if (recipe != null)
				env.world.setBlockAndUpdate(pos, CopyProperties.copyProperties(blockState, recipe.value.result))
		}
	}
}
