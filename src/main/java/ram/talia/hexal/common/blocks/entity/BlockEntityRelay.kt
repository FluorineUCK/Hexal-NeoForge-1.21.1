...
import ram.talia.hexal.hexcompat.HexPigmentCompat
...
class BlockEntityRelay(pos: BlockPos, val state: BlockState) : HexBlockEntity(HexalBlockEntities.RELAY, pos, state), ILinkable, ILinkable.IRenderCentre, GeoBlockEntity {
    ...
    fun debug() {
        ram.talia.hexal.Hexal.LOGGER.debug("relay network: {}", relayNetwork)
    }
    ...
    private fun getBobberPosition(): Vec3 {
        val manager = instanceCache.getManagerForId<BlockEntityRelay>(0L)
    ...
    override fun loadModData(tag: CompoundTag, registries: HolderLookup.Provider) {
        if (tag.contains(TAG_PIGMENT))
            relayNetwork.pigment = FrozenPigment.CODEC.parse(NbtOps.INSTANCE, tag.get(TAG_PIGMENT)).getOrThrow()
    ...
    override fun saveModData(tag: CompoundTag, registries: HolderLookup.Provider) {
        tag.put(TAG_PIGMENT, FrozenPigment.CODEC.encodeStart(NbtOps.INSTANCE, relayNetwork.pigment).getOrThrow())
        tag.putLong(TAG_PIGMENT_TIME, relayNetwork.timeColouriserSet)
        (linkableHolder?.writeToNbt() ?: serialisedLinkableHolder?.copy())?.let {
            tag.putCompound(TAG_LINKABLE_HOLDER, it)
        }
    ...
    private data class RelayNetwork(val root: BlockEntityRelay, val relays: MutableSet<BlockEntityRelay>, val nonRelays: MutableSet<ILinkable>, val mediaExchangers: MutableSet<ILinkable>) {
        ...
        var pigment: FrozenPigment = FrozenPigment(ItemStack(HexPigmentCompat.dyePigmentItem(DyeColor.PURPLE)), Util.NIL_UUID)
        ...
        fun canAcceptMedia(other: ILinkable, otherMediaLevel: Long): Long {
            ...
            return ((otherMediaLevel - averageMediaWithoutOther).mulBounded(HexalConfig.Server.MEDIA_FLOW_RATE_OVER_LINK.get()))
        }
