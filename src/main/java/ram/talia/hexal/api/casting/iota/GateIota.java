...
public class GateIota extends Iota {
    ...
    /**
     * Compatibility factories for Hexal 1.20 Everbook data.  The old iota codec stored an
     * entity anchor as UUID/name/offset and therefore cannot use the public constructor,
     * which expects the referenced entity to be loaded.
     */
    public static GateIota fromLegacyDrifting(int index) {
        return new GateIota(new Payload(index, Anyone.Companion.third(DriftingAnchor.INSTANCE)));
    }

    public static GateIota fromLegacyLocation(int index, Vec3 position) {
        return new GateIota(new Payload(index, Anyone.Companion.first(position)));
    }

    public static GateIota fromLegacyEntity(int index, UUID uuid, String name, Vec3 offset) {
        return new GateIota(new Payload(index, Anyone.Companion.second(new EntityAnchor(uuid, name, offset))));
    }
