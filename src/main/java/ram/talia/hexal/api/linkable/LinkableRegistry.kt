...
//			= PriorityQueue { type0, type1 -> type1.iotaPriority - type0.iotaPriority }
	...
	fun fromNbt(tag: CompoundTag, level: ServerLevel): Result<ILinkable> {
		...
		val resourceId = ResourceLocation.tryParse(typeId)
		if (resourceId == null)
			return Result.failure(InvalidLinkableTypeException("$typeId is not a valid resource location"))

		val type = linkableTypes[resourceId] ?: return Result.failure(InvalidLinkableTypeException("$typeId is not a registered linkable type"))
	...
	fun fromSync(tag: CompoundTag, level: Level): ILinkable.IRenderCentre? {
		...
		val type = linkableTypes[resourceId] ?: throw InvalidLinkableTypeException("no LinkableType registered for $typeId")
